 ...
 * @author     Sam Soffes
 * @author     Josh Turmel
 * @copyright  (c) 2007-2008 LifeChurch.tv
 */
class Pages_Core {
	...
	public function addTitle($title)
	{
		...
		// Manually reverse the title - lazy dev
		$title = array($this->title[1], $this->title[0]);
		$this->title = $title;
	}
	...
	/* ---------------------------------------------------------
		addView($nameOfView, $viewFile[, $config = false])
		
		Add an array of variables to be made avaliable in each
		view as $config['KEY_OF_EACH_ITEM']
	--------------------------------------------------------- */
	public function addView($nameOfView, $viewFile, $config = false) {
		
		// Add it to the array of views
		$this->views[$nameOfView] = page::view($viewFile, $config);
	}
	...
	public static function subView($view) {
		return @Kohana::instance()->page->views[$view];
	}

	public static function template($view, $data = false) {
		$page = Kohana::instance()->page;
		
		return page::view($view, $data);
	}
