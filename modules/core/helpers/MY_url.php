 ...
 * URL helper class.
 *
 * $Id: url.php 3917 2009-03-28 03:06:22Z joshshea $
 *
 * @package    Core
 * @author     Kohana Team
...
class url extends url_Core {
	 ...
	 * @return  string
	 */
	public static function base($index = FALSE, $protocol = FALSE, $subdomain = 'www')
	{
		...
		if ($protocol == FALSE)
		{
			...
			else
			{
				...
				$base_url = ((empty($_SERVER['HTTPS']) OR $_SERVER['HTTPS'] === 'off') ? 'http' : 'https').'://'.$subdomain.'.'.$site_domain;
			}
		}
		else
		{
			if ($site_domain === '' OR $site_domain[0] === '/')
			{
				...
				$base_url = $protocol.'://'.$_SERVER['HTTP_HOST'].$subdomain.'.'.$site_domain;
			}
			else
			{
				// Use the configured site domain
				$base_url = $protocol.'://'.$subdomain.'.'.$site_domain;
			}
		}
	 ...
	 * @return  string
	 */
	public static function site($uri = '', $protocol = FALSE, $subdomain = 'www')
	{
